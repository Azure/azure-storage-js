...
export class BufferScheduler {
  ...
  constructor(
    ...
    if (bufferSize <= 0) {
      throw new Error(`bufferSize must be larger than 0, current is ${bufferSize}`);
    }

    if (maxBuffers <= 0) {
      throw new Error(`maxBuffers must be larger than 0, current is ${maxBuffers}`);
    }

    if (parallelism <= 0) {
      throw new Error(`parallelism must be larger than 0, current is ${parallelism}`);
    }
      ...
      this.emitter.on("checkEnd", () => {
        ...
        if (this.isStreamEnd && this.executingOutgoingHandlers === 0) {
          if (this.unresolvedLength > 0 && this.unresolvedLength < this.bufferSize) {
            this.outgoingHandler(this.shiftBufferFromUnresolvedDataArray(), this.offset).then(() => {
              resolve();
            }).catch((err) => {
              reject(err);
            });
